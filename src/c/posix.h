	...
	char ***output, size_t *numOutput)
{
	...
	if (pipe(stdinPipe) == -1 || pipe(stdoutPipe) == -1) { error("pipe"); return ERROR_PIPE; }

	// Fork to create a child process
	pid_t pid = fork();

	if (pid == -1) { error("fork"); return ERROR_FORK; }
	...
	if (pid == 0) { // Child process
		...
		while ((bytesRead = read(stdoutPipe[0], buffer, sizeof(buffer))) > 0) {
			if (totalBytesRead + bytesRead > bufferSize) {
				...
				if (outputBuffer == NULL) { error("Memory reallocation failed (run_command)"); return ERROR_REALLOC; }
			}
